...
public class MemberController extends HttpServlet {
	...
	IOException {
		...
		if(uri.indexOf("list.do") != -1) {
			...
			String page="/project/login2.jsp";
			RequestDispatcher rd=request.getRequestDispatcher(page);
			rd.forward(request, response);
			...
			System.out.println(result);
			request.setAttribute("result", result);*/
			MemberDTO dto = dao.login_check(userid);
			if(dto!=null) {
				if(BCrypt.checkpw(passwd, dto.getPasswd())) {
					System.out.println(userid+" / "+dto.getName());
					HttpSession session = request.getSession();
					session.setAttribute("userid", userid);
					session.setAttribute("name", dto.getName());
					response.sendRedirect(context+"/project/login2.jsp?message=login");
				}else {
					response.sendRedirect(context+"/project/session_login.jsp?message=error");
				}
			}else {
				response.sendRedirect(context+"/project/session_login.jsp?message=error");
			}		
			...
			HttpSession session = request.getSession();
			session.invalidate();
			response.sendRedirect(request.getContextPath()+"/project/my_project.jsp?message=logout");
		}
	}
