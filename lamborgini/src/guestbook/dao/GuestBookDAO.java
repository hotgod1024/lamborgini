...
public class GuestBookDAO {
	...
	public int guestbookCount() {
		...
		int count=session.selectOne("guestbook.guestbookCount");
		session.close();
		return count;
	}
	...
	public List<GuestBookDTO> getList(String searchkey, String search,int start, int end){
		//mybatis 실행 객체
		SqlSession session=
				MybatisManager.getInstance().openSession();
		...
		if(searchkey.equals("name_content")) {//이름+내용
			...
			map.put("start", start);
			map.put("end", end);
			list=session.selectList("gbListAll", map);
